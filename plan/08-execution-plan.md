...
日期：2026-09-17。核对源码：`63f7d8e13319d57e946e46d5766703e55d75c9a3`（**历史基线**：本计划冻结时的源码）。
状态：本计划继续作为 C01—C09 约束与任务定义的真源；**§1 的实施前核对、§1.2 的本地记录是历史快照**，
不再代表当前状态。当前交付状态（2026-09-22）：RP00—RP14 已交付并原子提交（K1—K7 与 K6 第一阶段），
逐项实际验证记录见 [check/20260922-v3plan-execution-plan.md](../check/20260922-v3plan-execution-plan.md)；
RP15（C02 产品决策）、RP16（导航与总检查）、RP17（同 SHA 目标复验）未完，**功能与设备验收均未通过**。
...
> **历史快照（2026-09-17）**：下表是本计划冻结时的核对结论。其中“模块不存在”“未接入”“缺失”等判断
> 只对当时的基线成立；此后由 P02—P31 / RP00—RP14 交付的内容以各任务的实际验证记录为准
> （见 [check/20260922-v3plan-execution-plan.md](../check/20260922-v3plan-execution-plan.md)）。
...
| 控制协议测试 | （历史）工作区有未跟踪 `tests/test_control_protocol_v1.py`，对应模块不存在，收集时报 ImportError | P02 接续并评审此草稿；**该模块与测试现均在库**（`model_scheduler/control_protocol_v1.py`、`tests/test_control_protocol_v1.py`），P02/RP 系列已交付 |
...
（**历史**：这两个根 README 补丁文件已由 RP04/RP04a 以有界策略整合并原子提交——见 §1.2.1。）

### 1.2 本次本地核对记录（**历史**：09-17 的 3.13.5 核对，发布要求 3.12）
...
以下是本轮实施采用的设计决定；除节内以「RPxx 已交付」明确标注者外，它们不声称已实现——标注只说明
相应约束已落地并有其任务的验证记录，不代表设备或生产验收通过。与旧文件冲突处须由相应任务同步原章节和测试。
