...
### CT10b 根因确认与修复方案（2026-09-26，静态 + 数字核对；尚未改码）

- 复现链条（目标机，2026-09-26T04:28Z 同批材料）：请求 27B → docker 事件 `kill 7B`(…906) → `die/destroy` → `create 27B`(…907) →
  `start`(…908)；此后四次快照（跨度 ~50 s）7B 仍 `ready`/`generation=1` 而容器只剩 27B；再接 7B 请求 → 503
  `The input could not be counted against the envelope` → 7B `state=error`。llama-swap 完整日志无 7B 停止动作，调度器日志只有 4 条
  `GET /api/status`。
- **根因（用登记数字确认）**：候选/基线登记 `qwen25vl-7b.reserved_bytes=6_106_148_045`、`qwen36-27b.reserved_bytes=29_000_000_000`，
  `resources.model_budget_bytes=50_000_000_000` ⇒ 两者之和 35.1 GB **≤ 50 GB**，调度器的内存模型判定"可共存"，因此加载 27B 时
  `_load_deficit` 为 0、**不产生任何驱逐操作**（账本不动，7B 保持 READY）；而实际运行时是单实例语义（旧容器被停），于是
  **账本与现场分叉**：下一次 7B 请求在 READY 上被准入（`app.py:565` `acquire` 认为无需加载），计数阶段（`app.py:568`
  `chat_counter.count`）对着已消失的实例失败 → 503（`app.py:572`）→ 该模型被置 `error`。`ManagedLifecycle.load`
  （`backend_control.py:205`）与 `LifecyclePolicy`（`ports_v3.py:55`，仅时序）都不停别的模型，所以停止来自控制面/运行时的单实例语义。
- **修复方案（按用户裁定：切换后旧思考链丢失可接受；执行中禁止切换可接受）**：
  - **A（真话化登记，消除分叉的产生条件）**：把 `resources.model_budget_bytes` 降到 32 GB（> 单模型最大预留 29 GB，且 < 6.1+29=35.1 GB）
    ⇒ 任意跨模型加载都会产生缺口，必须**先产生账本驱逐操作**再加载；两个方向互斥（27B 需 29 ≤ 32 ✓；7B 需 6.1 ≤ 32 ✓；
    两者 35.1 > 32 ⇒ 驱逐）。这与运行时实际语义一致，且不需要夸大任何单模型预留。属于**冻结输入**变更 ⇒ 必须随候选切换一起重冻
    （配置摘要变 ⇒ manifest/llama-swap 重渲染、site 重冻）。
  - **B（代码加固，防线内移）**：`ModelScheduler.acquire` 在账本 READY 时必须确认实例**仍活着**（用既有观察端口有界采样）；对
    "实例已消失"的 READY 需要**重新收敛**（置为可加载状态并冷加载），而不是把租约交给计数；且实例消失导致的计数失败**不得**把模型
    永久置为 `error`。TDD 用例：(1) READY + 观察为 STOPPED ⇒ acquire 必须返回新实例的租约或给出可重试拒绝，账本不得停在
    "READY + 陈旧实例"；(2) 同一场景不得产生 `error` 终态；(3) 计数不得对着非 RUNNING 实例发生。
- **硬件验证判据（A+B 之后，重冻再跑）**：重放"7B → 请求 27B → 回切 7B"：切换后 7B 必须是 `unloaded` 且容器确实被停（账本与现场一致）；
  回切时先驱逐 27B、再冷加载 7B，**不得**出现 503 级联，也不得出现 `error`。
- 影响与本轮边界：本轮**未改任何代码或冻结文件**；A 会改变配置摘要，故与 B 一起做、随 CT10 重冻一并提交，避免出现"半改"状态。

### CT10b 修复方案修正（2026-09-26 续读代码后）：A 即可闭环，B 降为恢复体验项

- 读 `Book.acquire_ready`（`model_registry.py:290`）：门槛只有 `state is READY`、未阻断、并发未满——**不校验实例是否仍存在**，确认了"READY + 陈旧实例仍可被准入"这一缺口。
- 但计数区（`chat_counting.py`/`execution_service.py`/`gateway.py`）**没有任何** `book.failed`/`last_error` 写入，说明 7B 的 `error` 不是计数路径写的；
  结合 `backend_control.py:88-95`（K4 `BootLaunchRecords`："一旦 load 派发，记录保持 `starting` 直到终局裁决；**丢失的实例永远不会被读成已证明的停止**"），
  `error` 是**设计给出的**响应：实例在无人见证终局的情况下消失 ⇒ 停止无法被证明 ⇒ 该代被记为失败。
- 因此根因链的最终表述：**登记谎称可共存（35.1 ≤ 50 GB）⇒ 跨模型加载不驱逐 ⇒ 旧实例被运行时单实例语义停掉而无见证 ⇒ 按 K4 该代记为 `error`（准入被阻断）⇒ 级联**。
  **修好 A（预算降到 32 GB ⇒ 任何跨模型加载必须先做账本驱逐：排空租约 → 停止 → 证明 STOPPED → UNLOADED）就能让这条链根本不成立**，
  无需改动 K4/K5 的既有安全语义（"不把丢失当停止"本身是正确的，不应为图省事弱化）。
