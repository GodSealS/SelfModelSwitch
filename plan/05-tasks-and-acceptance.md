...
本表只实施 SelfModelSwitch 的模型挂载、切换和必要接口。
**当前进度（2026-09-22）**：下表的 M00—M07 是**实施前的任务快照**（“全部为待办”只对 09-16 成立）；
v3 的实施与修复进度、逐项实际验证记录见 [08 执行计划](08-execution-plan.md) 与
[本轮执行Plan](../check/20260922-v3plan-execution-plan.md)（RP00—RP14 已交付，RP15/RP17 待办，
仅当最终生产模型数 > 1 时需 RP11/RP12）。本表继续作为范围与交付定义；**设备与生产结论仍未验收**。
