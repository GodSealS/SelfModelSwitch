...
| `…/run-o1/`, `…/run-o1-all-503/`, `…/candidate-t1.json`, `…/policy-o1.json` | the first O-layer run: O01 failed with 120/120 requests answered `503 The input could not be counted against the envelope` (a compat-surface defect: counting runs before the load, so a cold model can never be reached), O05 refused by a device-tree mismatch (the live-site reader has since been aligned with `acceptance collect`), O02/O03/O04/O06 `not_run` (ports not wired). Material is split across the two directories because the output was renamed while the run was live; no `report.json` was written | recompute `run-o1/cases/o01/attempt-1` with `acceptance.evaluator.evaluate_case` — it re-derives `error_rate=1.0` from the workload rows and a clean end state |
